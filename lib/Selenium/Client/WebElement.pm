...
See the documentation for Selenium::Remote::WebElement for more details.
...
sub _execute_command($self,@args) {
    return $self->driver->_execute_command(@args);
}
...
sub send_keys {
    my ( $self, @strings ) = @_;
    die "no keys to send" unless scalar @strings >= 1;
...
sub get_attribute {
    ...
    if ( not defined $attr_name ) {
        die 'Attribute name not provided';
    }
...
sub get_css_attribute {
    my ( $self, $attr_name ) = @_;
    if ( not defined $attr_name ) {
        die 'CSS attribute name not provided';
    }
    my $res = {
